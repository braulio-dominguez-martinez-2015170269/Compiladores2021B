class Interpreter {
    ...
    visit_RoundNode(node, ctx) {
        ...
        if (!(arg instanceof BaseNumber)) {
            this.error(`Argument must be a number: REDONDEO(${arg.value})`, node.position)
        }
    ...
    visit_FloorNode(node, ctx) {
        const arg =  this.visit(node.arg, ctx)
        if (!(arg instanceof BaseNumber)) {
            this.error(`Argument must be a number: ABAJO(${arg.value})`, node.position)
        }
    ...
    visit_CeilNode(node, ctx) {
        const arg =  this.visit(node.arg, ctx)
        if (!(arg instanceof BaseNumber)) {
            this.error(`Argument must be a number: ARRIBA(${arg.value})`, node.position)
        }
    ...
    visit_SinNode(node, ctx) {
        const arg =  this.visit(node.arg, ctx)
        if (!(arg instanceof BaseNumber)) {
            this.error(`Argument must be a number: SENO(${arg.value})`, node.position)
        }
    ...
    visit_CosNode(node, ctx) {
        const arg =  this.visit(node.arg, ctx)
        if (!(arg instanceof BaseNumber)) {
            this.error(`Argument must be a number: COSENO(${arg.value})`, node.position)
        }
    ...
    visit_TanNode(node, ctx) {
        const arg =  this.visit(node.arg, ctx)
        if (!(arg instanceof BaseNumber)) {
            this.error(`Argument must be a number: TANGENTE(${arg.value})`, node.position)
        }
    ...
    visit_LogNode(node, ctx) {
        const arg =  this.visit(node.arg, ctx)
        if (!(arg instanceof BaseNumber)) {
            this.error(`Argument must be a number: LOGARITMO(${arg.value})`, node.position)
        }
    ...
    visit_ExpNode(node, ctx) {
        const arg =  this.visit(node.arg, ctx)
        if (!(arg instanceof BaseNumber)) {
            this.error(`Argument must be a number: EXPONENCIAL(${arg.value})`, node.position)
        }
    ...
    visit_PowerNode(node, ctx) {
        ...
        if (!(arg instanceof BaseNumber) || !(exp instanceof BaseNumber)) {
            this.error(`Argument must be a number: POTENCIA(${arg.value})`, node.position)
        }
    ...
    visit_SqrtNode(node, ctx) {
        const arg =  this.visit(node.arg, ctx)
        if (!(arg instanceof BaseNumber)) {
            this.error(`Argument must be a number: RAIZ(${arg.value})`, node.position)
        }
    ...
    visit_AbsNode(node, ctx) {
        const arg =  this.visit(node.arg, ctx)
        if (!(arg instanceof BaseNumber)) {
            this.error(`Argument must be a number: ABSOLUTO(${arg.value})`, node.position)
        }
    ...
    visit_ForNode(node, ctx) {
        ...
        if (!(forStart instanceof FloatNumber) || !(forEnd instanceof FloatNumber) || !(forStep instanceof FloatNumber)) {
            this.error(`Arguments must be floats: DESDE`, node.position)
        }
        if (forStep.value === 0) {
            this.error(`Step can't be zero: DESDE`, node.position)
        }
    ...
    visit_UnOpNode(node, ctx) {
        ...
        } else if (node.operator.tokentype === TokenType.KEYWORD && node.operator.value === "NO") {
            if (left instanceof IntNumber) {
                return new IntNumber(left.value === 0 ? 1 : 0)
            } else {
                this.error("Integer expected (NO)", left.position)
            }
        }
        return result
    }
